...
You job is to plot the Latin America data with the circle() glyph, and the Africa data with the x() glyph.
...
In this exercise, you'll plot female literacy vs fertility for Africa and Latin America as red and blue circle glyphs.
Instructions
...
In this exercise, you'll plot the daily adjusted closing price of AAPL stock from 2000 to 2013.
...
Your job is to plot longitude on the x-axis and latitude on the y-axis.

The figure object has been created for you as p.
...
100xp
In the previous exercises, we were making plots using data stored in lists. We learned that Bokeh can plot numbers and datetime objects.
...
Possible Answers
All columns in a ColumnDataSource must have the same length. 1
ColumnDataSource objects cannot be shared between different plots. 2
ColumnDataSource objects are interchangeable with Pandas DataFrames. 3
"""
...
Your job is to import the ColumnDataSource class, create a new ColumnDataSource object from the DataFrame df, and plot circle glyphs with 'Year' on the x-axis, 'Time' on the y-axis and color each glyph by the color column.
...
Use p.circle() to plot circle glyphs of size=8 on the figure p with 'Year' on the x-axis and 'Time' on the y-axis. Be sure to specify source=source and color='color' so that the ColumnDataSource object is used and each glyph is colored by the color column.
"""
# Import the ColumnDataSource class from bokeh.plotting
...
p.circle(x='Year',y='Time',color='color',source=source,size=8)
...
For this exercise you'll use the ColumnDataSource object of the Olympic Sprint dataset you made in the last exercise. It is provided to you with the name source.
...
Now that you have added 'box_select' to p, add in circle glyphs with p.circle() such that the selected glyphs are red and non-selected glyphs are transparent blue. This can be done by specifying 'red' as the argument to selection_color and 0.1 to nonselection_alpha. Remember to also pass in the x, y, and source arguments to p.circle().
