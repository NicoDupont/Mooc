...
"""
In this final chapter, you will build a more sophisticated Bokeh data exploration application from the ground up, based on the famous Gapminder data set.
"""
...
50xp
For the final chapter, we will be looking at some of the Gapminder datasets combined into one tidy file called "gapminder_tidy.csv". This data set is available as a Pandas DataFrame under the variable name data.
...
plot = figure(title='Gapminder Data for 1970', plot_width=700, plot_height=400,
...
""" Console Output or results
see plot43.png
Great work! Notice how life expectancy seems to go down as fertility goes up? It would be interesting to see how this varies by continent. In the next exercise, you will do this by shading each glyph by its continent.
"""
...
After you are done, you may have to scroll to the right to view the entire plot. As you play around with the slider, notice that the title of the plot is not updated along with the year. This is something you will fix in the next exercise!
Instructions
...
Remember how in the plot from the previous exercise, the title did not update along with the slider? In this exercise, you will fix this.

In Python, we can format strings by specifying placeholders with the % keyword. For example, if we have a string company = 'DataCamp', we can use print('%s' % company) to print DataCamp. Placeholders are useful when you are printing values that are not static, such as the value of the year slider. You can specify a placeholder for a number with %d. Here, when you are updating the plot title inside your callback function, you should make use of a placeholder so that the year displayed is in accordance with the value of the year slider.

In addition to updating the plot title, you will also create the callback function and slider as you did in the previous exercise, so you get a chance to practice these concepts further.
...
""" Console Output or results
see plot46.png
Excellent work! You will add even more interactivity to the app in the next exercises.
"""
...
In this exercise, you will practice adding a hover tool to drill down into data column values and display more detailed information about each scatter point.

After you are done, experiment with the hover tool and see how it displays the name of the country when your mouse hovers over a point!
